    ...
    async fn message(&self, ctx: Context, msg: Message) {
        ...
        if let Some(rest) = rest_opt {
            ...
            let result = match command_name.as_str() {
                ...
                "info" | "i"  => crate::commands::info::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "help" | "h"  => crate::commands::help::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "ignorechannel" | "ignore" => crate::commands::ignorechannel::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "bindchannel" | "bind" => crate::commands::bindchannel::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "adminbypass" | "bypass" => crate::commands::adminbypass::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "avatar" | "av" => crate::commands::avatar::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                ...
                "stop" | "st" => crate::commands::stop::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "join" | "j"  => crate::commands::join::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "leave" | "disconnect" | "dc" => crate::commands::leave::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                ...
                "shuffle" | "sh" => crate::commands::shuffle::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "remove" | "rm" => crate::commands::remove::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "move" | "mv" => crate::commands::move_cmd::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "clear" | "c" => crate::commands::clear::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "noprefix" | "nop" => crate::commands::noprefix::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "setprefix" | "prefix" => crate::commands::setprefix::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "24/7" | "247" => crate::commands::twenty_four_seven::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "lyrics" | "ly" => crate::commands::lyrics::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "premium"     => crate::commands::premium::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "autoplay" | "ap" => crate::commands::autoplay::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "serveravatar" | "sav" => crate::commands::serveravatar::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "serverbanner" | "sb" => crate::commands::serverbanner::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
                "serverbio" | "sbio" => crate::commands::serverbio::run(&ctx, &ctx_cmd, Arc::clone(&state), &args).await,
